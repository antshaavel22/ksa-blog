...
6. **Excerpts are original 2-sentence summaries of the whole article.** (Ants, 2026-08-14.) An `excerpt` is copy we write — **exactly two sentences** that catchily sum up what the *whole* post is about, in our own words. It is **never** sentences lifted or lightly reworded out of the body, never just a rehash of the opening, and never cut mid-sentence.
   - Rules live in **`lib/excerpt-rules.mjs`** — the single source of truth, imported by the audit, the batch generator, and the admin's ✨ Genereeri button so they can't drift apart.
   - Checks: exactly 2 sentences · complete ending (`.` `!` `?`) · 130–230 chars · correct language · not verbatim from the body · no banned words (*honest/aus/честно*, ICB) · no "this article…" meta phrasing.
   - `node scripts/audit-excerpts.mjs` — see what fails (add `--list` for filenames).
   - `node scripts/generate-excerpts.mjs [--dry-run] [--limit N] [--all]` — rewrite the failing ones.
   - In the editor, the **✨ Genereeri** button next to the excerpt field writes one for the open post.
   - `scripts/fix-excerpts.mjs` is the older repair-only tool (truncates/appends `...`); prefer regenerating, since a trailing `...` no longer satisfies this rule.
...
9. **Low-key trust voice (non-negotiable).** Calm, professional, never sales-y. No superlatives (*best, magic, revolutionary, perfect, life-changing, kõige parem, imetilk, лучший, чудо*). Cite study names + numbers instead of intensifiers. Two reasons: anxious readers read hype as marketing → broken trust; Estonian medical authorities + competing clinics monitor → superlative treatment claims invite complaints. CTAs handle conversion; text only informs. Exception: patient stories — enthusiastic patient quotes are fine, but only from the patient's mouth, never us narrating.
10. **Claude KSA Blog Editor (humanizer-ksa skill).** New 2026-04-27. Lives at `~/.claude/skills/humanizer-ksa/SKILL.md`. Layered on `~/.claude/skills/humanizer/` (blader/humanizer). Trilingual EE/RU/EN vocabulary lists of AI tells + KSA voice rules + Jana/Silvia/Haavel hand-off. Run on every AI-generated draft before it reaches editors — they should receive ~90% publish-ready material. Reference voice: `content/drafts/et/2026-04-22-vahiravi-ja-sinu-silmade-tervise-raakimata-lugu.mdx` (rewritten + Haavel-approved 2026-04-27). Strategic context: blog is 20–30% of new leads + free-value channel for existing patients → word-of-mouth + referrals. Win-win-win.
...
## Changelog (session 2026-04-30)
- **Vercel build unblock + future-proofing:** Production was red since 2026-04-30 13:01 because the post `2026-04-21-vitreous-floaters-tiny-black-dots-on-white-walls.mdx` had a raw Rendia HTML embed (`<var style="..." data-presentation="UUID">...</var><script src="//hub.rendia.com/whitelabel/embed.js"></script>`) pasted into its body. React rejects HTML-string `style="..."` attributes in MDX, so prerender exited code 1 on every deploy after that commit. Replaced the raw block with `<RendiaEmbed id="UUID" caption="Source: Rendia" />` (commit `1615e0f`). Then hardened `scripts/validate-content.ts` (the prebuild gate) so this can never recur:
  - Auto-repairs any raw Rendia embed pattern across `content/posts/` + `content/drafts/` to the MDX component on every prebuild — keeps the build green even if a stray raw embed slips into a future MDX file.
  - Hard-fails the build if any post body still contains a raw HTML `style="..."` attribute, with the offending file path + line + snippet shown in the build log. Editor-side problem is now surfaced before Vercel ever attempts to prerender.
